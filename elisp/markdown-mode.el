...
Square brackets are not included in the returned labels."
  (save-excursion
    (goto-char (point-min))
    (let (refs)
      (while (re-search-forward markdown-regex-reference-definition nil t)
  ...
  (save-excursion
    (if (fboundp 'edit-indirect-region)
        (let* ((bounds (markdown-get-enclosing-fenced-block-construct))
               (begin (and bounds (goto-char (nth 0 bounds)) (line-beginning-position 2)))
               (end (and bounds (goto-char (nth 1 bounds)) (line-beginning-position 1))))
          (if (and begin end)
